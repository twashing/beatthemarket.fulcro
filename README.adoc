...
# Compile the CLJS
lein run -m shadow.cljs.devtools.cli release main
...
# Browser REPL
shadow-cljs browser-repl main

;; Run the connect-websocket function in the browser console (after going to localhost:3000)
> beatthemarket.client.connect_websocket()
```
...
Using shadow-cljs through deps.edn
```
λ clj -A:shadow-cljs clj-repl main  # OR
λ clj -A:shadow-cljs watch main
```

Using main code through deps.edn
```
λ clj -A:dev -C:deps -Sdeps '{:deps {nrepl {:mvn/version "0.6.0"}}}' -m nrepl.cmdline --interactive
```



NOTE: The YouTube videos and Developer's Guide are slightly out of date with respect to this template.  This template
now uses Pathom for server-side mutation and query handling.  See `beatthemarket/server_components/pathom_wrappers.clj` and
`beatthemarket/model/user.clj` for the macro definitions and examples of how to define
...
`src/main/beatthemarket/client.cljs` contains the code to start and refresh
...
See the server section below for working on the full-stack beatthemarket itself.
...
application `beatthemarket.development-preload`. You can add code here that
...
```
lein uberjar
java -jar target/beatthemarket.jar
```
